...
train_config=conf/tuning/train_pytorch_transformer.hayashi.yaml 
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    # utils/combine_data.sh --extra_files utt2num_frames data/${train_set_ori}_org data/train_clean_100 data/train_clean_360 data/train_other_500
    ...
    for x in ${train_set}; do
        steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 128 --write_utt2num_frames true \
                                  data/${x} exp/make_fbank/${x} ${fbankdir}
        utils/fix_data_dir.sh data/${x}
    ...
    # # compute global CMVN
    # compute-cmvn-stats scp:data/${train_set_ori}/feats.scp data/${train_set_ori}/cmvn.ark
    ...
    #     ${feat_dt_dir}/storage
    # fi
    dump.sh --cmd "$train_cmd" --nj 128 --do_delta ${do_delta} \
        data/${train_set}/feats.scp data/${train_set_ori}/cmvn.ark exp/dump_feats/train ${feat_tr_dir} &
    # dump.sh --cmd "$train_cmd" --nj 128 --do_delta ${do_delta} \
...
dict=data/lang_char/${train_set_ori}_${bpemode}${nbpe}_units.txt
bpemodel=data/lang_char/${train_set_ori}_${bpemode}${nbpe}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    # cut -f 2- -d" " data/${train_set_ori}/text > data/lang_char/input.txt
